...
class HNM_CWidgetHostNavigator extends CWidget {

	/**
	 * Host navigator instance.
	 *
	 * @type {HNM_CHostNavigator|null}
	 */
	#host_navigator = null;
	...
	setContents(response) {
		...
		if (this.#host_navigator === null) {
			...
			this.#host_navigator = new HNM_CHostNavigator(response.config);
	...
	#activateListeners() {
		this.#host_navigator.getContainer().addEventListener(HNM_CHostNavigator.EVENT_HOST_SELECT,
			this.#listeners.hostSelect
		);
		this.#host_navigator.getContainer().addEventListener(HNM_CHostNavigator.EVENT_GROUP_TOGGLE,
