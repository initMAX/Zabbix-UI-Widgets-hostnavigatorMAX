...
class HNM_CHostNavigator {

	static ZBX_STYLE_CLASS =		'host-navigator';
	static ZBX_STYLE_LIMIT =		'host-navigator-limit';
	static ZBX_STYLE_SEARCH =		'host-navigator-search';
	static ZBX_STYLE_SEARCH_INNER =	'host-navigator-search-inner';
	static ZBX_STYLE_SEARCH_CLEAR =	'host-navigator-search-clear';
	static ZBX_STYLE_NO_RESULTS =	'host-navigator-no-results';
	...
	/**
	 * Content container element (below search field, cleared on refresh).
	 *
	 * @type {HTMLElement}
	 */
	#content_container;
	...
	/**
	 * Raw data from the last setValue call (used for re-filtering).
	 *
	 * @type {Object|null}
	 */
	#raw_data = null;
	...
	/**
	 * Current search term (lower-cased).
	 *
	 * @type {string}
	 */
	#search_term = '';
	...
	constructor(config) {
		...
		this.#container.classList.add(HNM_CHostNavigator.ZBX_STYLE_CLASS);
	...
	setValue({hosts, maintenances, is_limit_exceeded, selected_hostid}) {
		this.#raw_data = {hosts, maintenances, is_limit_exceeded, selected_hostid};
		this.#applyFilter();
	}

	/**
	 * Filter stored hosts by the current search term and re-render the tree.
	 */
	#applyFilter() {
		this.#resetContent();

		const {hosts, maintenances, is_limit_exceeded, selected_hostid} = this.#raw_data;

		const filtered_hosts = this.#search_term !== ''
			? hosts.filter(host => host.name.toLowerCase().includes(this.#search_term))
			: hosts;

		if (filtered_hosts.length === 0) {
			const no_results = document.createElement('div');
			no_results.classList.add(HNM_CHostNavigator.ZBX_STYLE_NO_RESULTS);
			no_results.textContent = t('No results found');
			this.#content_container.appendChild(no_results);
			return;
		}

		this.#maintenances = maintenances;

		this.#prepareNodesStructure(filtered_hosts);
		...
		this.#content_container.appendChild(this.#navigation_tree.getContainer());

		if (is_limit_exceeded && this.#search_term === '') {
	...
	#prepareNodesProperties(nodes) {
		for (let i = 0; i < nodes.length; i++) {
			if (nodes[i].children === undefined) {
				...
				nodes[i].severity_filter = nodes[i].group_by.attribute === HNM_CHostNavigator.GROUP_BY_SEVERITY
	...
	#createGroup(host, level = 0, parent = null) {
		...
		switch (attribute.attribute) {
			case HNM_CHostNavigator.GROUP_BY_HOST_GROUP:
				for (const hostgroup of host.hostgroups) {
					const new_group = {
						...HNM_CHostNavigator.#getGroupTemplate(),
						name: hostgroup.name,
						group_by: {
							attribute: HNM_CHostNavigator.GROUP_BY_HOST_GROUP,
							name: t('Host group')
						},
			...
			case HNM_CHostNavigator.GROUP_BY_TAG_VALUE:
				...
				if (matching_tags.length === 0) {
					const new_group = {
						...
						group_by: {
							attribute: HNM_CHostNavigator.GROUP_BY_TAG_VALUE,
			...
			case HNM_CHostNavigator.GROUP_BY_SEVERITY:
				...
				if (!has_problems) {
					const new_group = {
						...
						name: t('Uncategorized'),
						group_by: {
							attribute: HNM_CHostNavigator.GROUP_BY_SEVERITY,
							name: t('Severity')
						},
						group_identifier: parent !== null ? [...parent.group_identifier, null] : [null],
						level,
	 ...
	 */
	#sortGroups(groups) {
		if (groups[0].group_by.attribute === HNM_CHostNavigator.GROUP_BY_SEVERITY) {
	...
	#createLimit(limit) {
		...
		element.classList.add(HNM_CHostNavigator.ZBX_STYLE_LIMIT);
	...
	#registerListeners() {
		this.#listeners = {
			hostSelect: e => {
				this.#container.dispatchEvent(new CustomEvent(HNM_CHostNavigator.EVENT_HOST_SELECT, {
					detail: {
						hostid: e.detail.id
					}
				}));
			...
			groupToggle: e => {
				...
				this.#container.dispatchEvent(new CustomEvent(HNM_CHostNavigator.EVENT_GROUP_TOGGLE, {
					detail: {
	...
	/**
	 * Create the search field and prepend it to the main container.
	 */
	#createSearchField() {
		const wrapper = document.createElement('div');
		wrapper.classList.add(HNM_CHostNavigator.ZBX_STYLE_SEARCH);

		this.#search_input = document.createElement('input');
		this.#search_input.type = 'text';
		this.#search_input.placeholder = t('Search hosts…');
		this.#search_input.setAttribute('aria-label', t('Search hosts'));

		const clear_btn = document.createElement('button');
		clear_btn.type = 'button';
		clear_btn.classList.add('btn-icon', 'zi-remove-small', HNM_CHostNavigator.ZBX_STYLE_SEARCH_CLEAR);
		clear_btn.setAttribute('aria-label', t('Clear search'));
		clear_btn.hidden = true;

		this.#search_input.addEventListener('input', () => {
			this.#search_term = this.#search_input.value.toLowerCase();
			clear_btn.hidden = this.#search_term === '';
		...
		clear_btn.addEventListener('click', () => {
			...
			this.#search_input.dispatchEvent(new Event('input'));
			this.#search_input.focus();
		});

		const inner = document.createElement('div');
		inner.classList.add(HNM_CHostNavigator.ZBX_STYLE_SEARCH_INNER);
		inner.appendChild(this.#search_input);
		inner.appendChild(clear_btn);
	...
	/**
	 * Clear the content container and reset tree-related state.
	 */
	#resetContent() {
